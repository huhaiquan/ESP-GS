...
public class Set_RandFailureEvaluation_main {
	...
	public static void main(String[] args) throws CloneNotSupportedException, IOException {
		...
		BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(
				new FileOutputStream("ComparisonResult_RandFailure.csv", true)));

			ConstNum.nBaseStation = 600; // 600(400-1000)
			ConstNum.nServer = 100; // 100(60-180)
...
//				System.out.println("第" + (time + 1) + "次重复试验");
				...
				bw.append(item.getAlgorithm().getClass().getSimpleName() + "," + result.getObjValue() + ","
						...
						+ "," );
				// Random Failure Test
				for (double index = 0.1; index <= 0.9; index += 0.1) {
					ConstNum.failureRate = index;
					
					double[] testResult = RandFailureTestUtils.execute(result, userListOfBS, randFailureTestTimes);
					bw.append(index + "," + testResult[0] + "," + testResult[1] + "," + testResult[2] + ",");
				
				}
				item.addResult(time + 1, result);
